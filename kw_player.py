...
class Player:
    def __init__(self, topics):
        ...
        while True:
            ...
            if self.mode == 'normal':
                ...
                while True:
                    topic = input(f'Choose topic, q to quit: {topics}: ')
                    if topic in topics:
    ...
    def update_stat(self, time):
        ...
        self.player_data[self.name] = {'level':self.level, 'xp':self.xp, 'removed':self.removed, 'already_answered':self.already_answered, 'answering':self.answering, 'boss_questions':self.boss_questions, 'last_played':datetime.datetime.strftime(datetime.datetime.now(), "%Y-%m-%d %H:%M:%S")}
        ...
        try:
            ...
            contents[datetime.datetime.strftime(datetime.datetime.strptime(str(time)))] = self.report
