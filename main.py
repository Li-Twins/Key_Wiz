...
class BookScreen(Screen):
    ...
    def load_books(self):
        book_dir = os.path.join(os.path.dirname(__file__), 'assets', 'book')
...
class PhotoGalleryScreen(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        Clock.schedule_once(lambda dt: self.load_photos(), 0.1) 
        
    def load_photos(self):
        photo_dir = os.path.join(os.path.dirname(__file__), 'assets', 'photo')
        ...
        def update_thumbnail_size(instance, value):
            # Recalculate on window resize
            thumbnail_size = (instance.width - 50) / 4  # Account for padding and spacing
            for child in instance.children:
                child.size = (thumbnail_size, thumbnail_size)
        
        # Bind the size calculation
        container.bind(width=update_thumbnail_size, size=update_thumbnail_size)
        
        def update_layout(instance, value):
            # Calculate available width minus padding
            available_width = instance.width - 20  # Account for padding
            thumbnail_size = (available_width - 30) / 4  # Account for spacing (3 gaps between 4 columns)
            ...
            rows = math.ceil(len(photo_files) / 4)
            container.height = rows * (thumbnail_size + 10)  # Account for spacing and padding
        
        # Initial calculation
        update_layout(container, None)
        
        # Bind to size changes
        container.bind(size=update_layout, parent=update_layout)

        # Create thumbnails
        for photo_file in photo_files:
            thumb_container = FloatLayout(
                size_hint=(None, None),
            )        
        
            # Add the image (centered)
            img = AsyncImage(
                source=os.path.join(photo_dir, photo_file),
                size_hint=(1, 1),
                pos_hint={'center_x': 0.5, 'center_y': 0.5},
                keep_ratio=True,
                allow_stretch=True,
            ...
            # Add the clickable button (full size of container)
            btn = Button(
                size_hint=(1, 1),
                pos_hint={'x': 0, 'y': 0}, # this ensures that the button is not repositioned relative to the thumbnail
                background_normal='',
                background_color=(0, 0, 0, 0),  # Fully transparent
                on_press=lambda instance, file=photo_file: self.show_full_image(file)
            )
                
            thumb_container.add_widget(img)
            thumb_container.add_widget(btn)
            container.add_widget(thumb_container)

        update_thumbnail_size(container, None)
        
    def show_full_image(self, filename):
        # Create a modal view to display the full image
        photo_dir = os.path.join(os.path.dirname(__file__), 'assets', 'photo')
        ...
        # Create a FloatLayout to hold the image and close button
        layout = FloatLayout()
        
        # Add the image (centered and properly sized)
        img = AsyncImage(
            source=filepath,
            keep_ratio=True,
            allow_stretch=True,
        ...
        def update_image_size(instance, value):
            img_width = min(instance.width, instance.height * img.image_ratio)
            img_height = min(instance.height, instance.width / img.image_ratio)
            img.size = (img_width, img_height)
        
        layout.bind(size=update_image_size)
        layout.bind(pos=update_image_size)
